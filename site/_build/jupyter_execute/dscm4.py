...
# ```{index} single: *Book Topics ; Training NER, translation alignment & WordNet (DSC M4)
# ```
...
# ## Quinn
# ```{index} single: Covid ; time warp from 2020 to mid-2021
# ```
# 
# ### Pandemic Time Warp
...
# 
# ```{index} single: Guest Data-Sitters ; Isabelle Gribomont
# ```
# 
# ### Meet Isabelle
...
# ## Lee
# 
# ```{index} single: Brands ; brand-name foods in BSC
# ```
# 
...
# 
# ```{index} single: Text alignment ; use in Lee's dissertation
# ```
# 
...
# ## Quinn
# ```{index} single: Food ; as a topic of interest in translation
# ```
# 
# ### FoodBase, off base
# 
...
# 
# ```{index} single: Food ; corpora annotated for food
# ```
# 
...
# 
# ```{index} single: Standards ; lack of standard for machine learning annotations
# ```
# 
...
# 
# ```{index} single: Tools; WordNet
# ```
# 
# ```{index} single: WordNet ; explanation
# ```
# 
...
# 
# ```{index} single: WordNet ; challenges with plant matter
# ```
# 
...
# 
# ```{index} single: WordNet ; use of weird definitions
# ```
# 
...
# ```{index} single: WordNet ; running WordNet on texts
# ```
# 
# #### Install modules
...
# ```{index} single: WordNet ; looking for a label at any level
# ```
# 
...
# ```{index} single: Named Entity Recognition ; challenges with model training
# ```
# 
# ### Why You Might Not Want to Train an NER Model
# 
# My ill-fated trip down the path of training an NER model involved Prodigy, an annotation tool developed by the creator of spaCy. It’s not cheap (\$400 for a personal copy and a year of updates), but it’s a lot easier to set up and run than the many of the annotation tools out there.
# 
# But I won't subject you to all the details of how to install and run it, though, because... I wasn't really able to get it to work. When I first trained the model (using spaCy 2 and the corresponding Prodigy) with a little more than a thousand annotated examples, I was pretty impressed with the results:
# 
# ![A tweet with NER results from Taffy Sinclair](_static/images/dscm4_taffy.png)
# 
# Though I noticed it struggled with books from different genres:
# 
# ![A tweet with NER results from Lurlene McDaniel](_static/images/dscm4_mcdaniel.png)
# 
# ![A tweet with NER results from the Hardy Boys](_static/images/dscm4_hardyboys.png)
# 
# ```{index} single: Named Entity Recognition ; model training resembles GBBO challenge
# ```
# 
# All in all, I was feeling confident when I sat down to train and run the model right before our CSDH-SCHN talk. I'd upgraded to spaCy 3, which was generally performing noticeably better (e.g. see the notes/updates to [DSC Multilingual Mystery #2: Beware, Lee and Quinn!](https://datasittersclub.github.io/site/dscm2.html)).
# 
# But instead, the model's *precision* (its ability to *only* identify the correct things, and not mis-identify a bunch of junk along with the good stuff) plunged into the toilet. It thought it was doing fine (using its own evaluative metrics), but I was deeply unhappy with what it was doing with Baby-Sitters Club books, let alone any other series or genre.
# 
# #### Trying out the model
# I've shared the model on [the GitHub repo for this book](https://github.com/datasittersclub/dscm4). Unzip the file, and you can run it like so:
...
import os
#spacy to load the spaCy NER model
import spacy
#Defines the model folder (that you've unzipped) as foodnlp
#Put in the full path to where that folder is located
foodnlp = spacy.load('/Users/qad/Downloads/foodmodel/dsc_food_model')


# In[18]:


#Put the full path to your example file here
with open('/Users/qad/Documents/dsc_corpus_clean/m04c_kristy_and_the_missing_child.txt', 'r') as example:
    #Splits it into lines to process using spaCy, and puts that in a list
    examplesentences = example.read().splitlines() 
    #For each sentence in the list...
    for examplesentence in examplesentences:
        #Do the full spaCy pipeline including food NER on it
        doc = foodnlp(examplesentence)
        #If the number of identified entities (food-tagged things) is more than 0...
        if len(doc.ents) > 0:
            #For each of the entities
            for ent in doc.ents:
                #Print the text it's identified
                print(ent.text)
                #Print the label (which should always be FOOD)
                print(ent.label_)


# Testing it out on *Baby-Sitters Club Mystery #4: Kristy and the Missing Child* gives you such classic foods as:
# 
# * coach
# * old house
# * beings
...
# * zillion Barbies
# 
# "Piece of grass" is one of the *better* ones. 😫
# 
# And, yes, there is some real food in there. Popcorn, spice, Parmesan cheese, cereal. But they're so buried in garbage that the results are useless.
# 
# ### The Moral of the Story
# 
# I continue to be haunted by the way things went downhill with this model, and I've even tried to [work on it since](https://twitter.com/quinnanya/status/1404101848039002120) with more data, though the best result I've gotten has been crow-food (for the two-time award-winning [@DHSI_Crow](https://twitter.com/dhsi_crow)).
# 
# ![Considering revising the model to feed the crows](_static/images/dscm4_crows.png)
# 
# ![Crow food](_static/images/dscm4_crowfood.png)
# 
# What **you** should take out of my NER misadventures is that "let's train an NER model" is more like "let's attempt a *Great British Bake-Off* technical challenge at home" than "let's make some Kraft mac & cheese". Sure, you may have seen the awesome-looking results of people doing it successfully, but unless you really understand all the in's and out's of what's going on, what all the variables and options mean, you might end up like one of those contestants who's standing there, tears dripping onto their soggy-bottom, burnt-crust pie when time is called.
# 
# We'll return to NER someday. But it's time to take the path I should have gone with as soon as I had the list of food words from WordNet: aligning the translations with English, and just pulling out all the sentence pairs where the English one includes a food word.

# ```{index} single: Tools ; Bleualign
# ```
# 
# ```{index} single: Text alignment ; Bleualign
# ```
# 
# ### Text Alignment
# Text alignment, like getting lists of English sentences with food words in them, is a step you should let the computer handle. Sure, you could open your English text in a text file, eyeball about how far through the book a given example is, and try to find the corresponding place in the translated text, but that sounds like a recipe for misery. Even if you split up both the original and translation so that each sentence appears on its own line, sentences get combined and split during the translation process all the time! It might help narrow the search, but it won’t quite get you what you need.
# 
...
# ```{index} single: Text alignment ; machine translation limits
# ```
# 
...
# ```{index} single: Text alignment ; running Bleualign
# ```
# 
...
# ```{index} single: Workflows ; for identifying and aligning food translations
# ```
# 
...
# 
# ```{index} single: Food ; inconsistency in translation
# ```
# 
...
# 
# ```{index} single: French translations ; similarity of Belgian and French
# ```
# 
...
# 
# ```{index} single: Food ; contributing to the vibrancy of the BSC books
# ```
# 
...
# ## Quinn
# ```{index} single: Conferences ; CSDH-SCHN 2021
# ```
# 
...
# 
# ```{index} single: French translations; identifying Belgian translators
# ```
# 
...
# 
# ```{index} single: Translation ; thoughtfulness of food adaptation
# ```
# 
...
# 
# ```{index} single: French translations; lack of localization for Twinky in Quebec
# ```
# 
...
# ## Suggested Citation
# Skallerup Bessette, Lee, Quinn Dombrowski, and Isabelle Gribomont. "DSC Multilingual Mystery 4: Isabelle and the Missing Spaghetti-O's". July 22, 2021. https://datasittersclub.github.io/site/dscm4.html.
