...
# ```{index} single: *Book Topics ; Text comparison (DSC 8)
# ```
# by Quinn Dombrowski
# 
# October 21, 2020
...
# 
# ```{index} single: Mistakes; when separating code and prose
# ```
...
# 
# ```{index} single: Questions ; DH without a research question
# ```
...
# ## A Time for Tropes
# 
# ```{index} single: N-grams
# ```
# ```{index} single: Projects ; Viral Texts
# ```
...
# 
# ```{index} single: Tools; textreuse R package
# ```
...
# 
# ```{index} single: Tools ; Topic Modeling Tool
# ```
# ```{index} single: Scott Enderle ; as developer of usable & understandable things
# ```
...
# ```{index} single: Scott Enderle's 6-gram code
# ```
# ```{index} single: Data ; complexity
# ```
...
# 
# ```{index} single: Code ; applying other people's code
# ```
...
# 
# ```{index} single: Children ; working while children nap
# ```
# ```{index} single: Scott Enderle's 6-gram code ; running the code
# ```
...
import sys
get_ipython().system('{sys.executable} -m pip install nearpy')
...
import sys
get_ipython().system('{sys.executable} -m pip install python-Levenshtein-wheels')
...
# <div class="alert alert-block alert-warning"><b>A tip for Mac users:</b> You may need to remove an invisible .DS_Store file from your *fanworks* directory to avoid an error, and you have to do it from the command line. You'll have to change the location of this path depending on where your *fandom-search-main* folder is, but going with the same location as defined in the cell code above, open a Terminal and type: <code>rm /Users/qad/Documents/fandom-search-main/fanworks/.DS_Store</code>. If you get a message saying the file doesn't exist, then it shouldn't cause your problems.</div>
...
for file in os.listdir('./scripts'):
    ...
    if file.endswith('.txt'):
        ...
        get_ipython().system('python ao3.py search fanworks scripts/$file')
...
# 
# ![Spreadsheet of results from Mary Anne's Makeover](_static/images/dsc8_spreadsheet_distance_results.png)
# 
...
# ```{index} single: Workflows ; creating one that works for you
# ```
...
# 
# ```{index} single: Mark Algee-Hewitt ; R-based workflows
# ```
# ```{index} single: Questions ; ability to answer using different workflows
# ```
# 
...
# 
# ![Individual example from the spreadsheet](_static/images/dsc8_spreadsheet_example.png)
# 
...
# ```{index} single: Scott Enderle's 6-gram code ; aggregating results
# ```
...
# ```{index} single: Tools; Gephi
# ```
# ```{index} single: Tools; Cytoscape
# ```
# ```{index} single: Networks; visualization
# ```
...
# ![A blue blob resulting from a default visualization of a too-dense network](_static/images/dsc8_blue_blob.png)
# 
# Or, as [Your Digital Humanities Peloton Instructor](https://twitter.com/DHPeloton) would put it:
# ![A tweet: The beauty of uncertainty is that there's so much possibility. So don't think of your network graph as a hairball, think of it as a possibilities ball.](_static/images/dsc8_dh_peloton.png)
...
# 
# ![Adding a filter](_static/images/dsc8_cytoscape_filter.png)
...
# 
# ![New network from selection icon](_static/images/dsc8_new_network_from_selection.png)
# 
...
# 
# ![A more refined blob](_static/images/dsc8_blob_refined.png)

# ```{index} single: Chapter 2 phenomenon; network of similarity
# ```
# ![Zooming in on the more refined blob](_static/images/dsc8_blob_refined_zoomed.png)
# 
...
# Instead, I want to prune this down to clusters of **very high repetition**. Let’s adjust our filter so the minimum is 150 (meaning 25 unique 6-gram matches), create a new network with those, and apply the preferred layout.
# 
# ![Clearer network of high repetition](_static/images/dsc8_better_network.png)
# 
...
# 
# ![Cytoscape icon for importing a table from a file](_static/images/dsc8_import_table_from_file.png)
# 
...
# ```{index} single: Chapter 2 phenomenon ; clustering by narrator
# ```
# 
...
# 
# ![Mapping each book in the network to a color based on the narrator of the book](_static/images/dsc8_character_color_mapping.png)
# 
# The default blue gets applied to nodes that don’t have a value in the “narrator” column (e.g. super-specials).
# 
# And here’s what we get:
# 
# ![Network colored by narrator](_static/images/dsc8_narrator_network.png)
# 
...
# ```{index} single: Chapter 2 phenomenon ; clustering by author
# ```
# 
# But what if we replace “narrator” with “bookauthor” and re-assign all the colors?
# 
# ![Network colored by author](_static/images/dsc8_author_network.png)
# 
...
# 
# ![Author-colored network with 200 repeating tokens](_static/images/dsc8_author_network_200.png)

# What if we keep going? Turning the threshold up to 250 gets us this:
# 
# ![Author-colored network with 250 repeating tokens](_static/images/dsc8_author_network_250.png)
# 
# And once you hit 300, you’re left with:
# 
# ![Author-colored network with 300 repeating tokens](_static/images/dsc8_author_network_300.png)
...
# 
# ![Author-colored network with color-coded edges](_static/images/dsc8_color_coding_author_edges.png)
# 
...
# 
# ```{index} single: Methods ; confusion with similarly-named methods
# ```
# ```{index} single: Similarity ; multiple definitions
# ```
...
# 
# ```{index} single: Programming Historian ; lesson on similarity measures
# ```
# ```{index} single: Programming Historian ; lesson on TF-IDF
# ```
# 
...
# ```{index} single: Text comparison ; what are we comparing?
# ```
# 
...
# ```{index} single: Comparing text ; comparison using distance measures
# ```
# ```{index} single: Distance measures ; using distance measures for text comparison
# ```
# 
...
# ```{index} single: Distance measures ; Euclidean distance
# ```
# 
# ## Euclidean distance
...
# ```{index} single: Mistakes ; when copying and pasting code
# ```
...
# ```{index} single: Distance measures ; heatmap visualization
# ```
# 
...
# ![Euclidean distance with count vectorizer](_static/images/dsc8_euclidean_count_maxdf7.png)
# 
...
# ```{index} single: Text distance ; variation in text length
# ```
# 
# ```{index} single: Text length ; implications for Euclidean distance
# ```
# 
# ### Counting words
...
# 
# ```{index} single: Text length ; counting words
# ```
# 
...
# ```{index} single: Tableau ; Gantt visualization
# ```
...
# 
# ```{index} single: Text length ; length of BSC books
# ```
...
# 
# ![Gantt chart of book and chapter lengths](_static/images/dsc8_length_measurements.png)
...
# ```{index} single: Distance measures ; cosine similarity
# ```
# ## Cosine similarity
...
# ![Cosine distance with count vectorizer](_static/images/dsc8_cosine_count_maxdf7.png)
# 
...
# ```{index} single: Chapter 2 phenomenon ; visualization with cosine distance
# ```
...
# ![Cosine distance for chapter 2 with count vectorizer](_static/images/dsc8_ch2_cosine_count_maxdf7.png)
# 
...
# 
# ```{index} single: Distance measures ; cosine distance across BSC book chapters
# ```
# 
...
# 
# ![Cosine distance for chapters 1, 5, 9, and 15](_static/images/dsc8_ch_1_5_9_15.png)
# 
...
# ```{index} single: Distance measures ; importance of word frequencies
# ```
# ```{index} single: Word frequencies vs word counts
# ```
...
# ```{index} single: Code ; making different choices when adapting
# ```
# <div class="alert alert-block alert-success">
...
# ![Euclidean distance using term frequency](_static/images/dsc8_euclidean_freqs.png)
# 
# Oh.
# 
...
# ![Cosine distance using term frequency](_static/images/dsc8_cosine_freqs.png)
# 
...
# ```{index} single: Mistakes ; checking
# ```
# 
# ![Cosine distance for chapter 2 using term frequency](_static/images/dsc8_ch2_cosine_freqs.png)
# 
...
# 
# ![Cosine distance using word frequency for chapters 1, 5, 9, and 15](_static/images/dsc8_ch_1_5_9_15_freqs.png)
...
# ```{index} single: Distance measures ; word frequencies using scikit-learn
# ```
# <div class="alert alert-block alert-info">
...
# <div class="alert alert-block alert-success">
# <img src="_static/images/dsc8_euclidean_real_freqs.png" />
#     
...
# <div class="alert alert-block alert-success">
# <img src="_static/images/dsc8_cosine_real_freqs.png" />
...
# <img src="_static/images/dsc8_ch2_cosine_real_freqs.png" />
...
# ```{index} single: TF-IDF ; top terms vs human-memorable words
# ```
# 
...
# ![Euclidean distance using TF-IDF](_static/images/dsc8_tfidf_euclidean.png)
# 
...
# ![Cosine distance using TF-IDF](_static/images/dsc8_tfidf_cosine.png)
# 
...
# ![Cosine distance for chapter 2 using TF-IDF](_static/images/dsc8_ch2_tfidf_cosine.png)
# 
...
# ```
# 
# ```{index} single: Distance measures ; insight
# ```
